...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit Sample Type', href: '#' },
];

export default function LabSampleTypeEdit({ labSampleType }: { labSampleType: LabSampleType }) {
    const { data, setData, put, processing, errors } = useForm({
        name: labSampleType.name,
        code: labSampleType.code,
        description: labSampleType.description ?? '',
        default_container: labSampleType.default_container ?? '',
        default_volume: labSampleType.default_volume ?? '',
        is_active: labSampleType.is_active,
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/lab-sample-types/${labSampleType.id}`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit Lab Sample Type: ${labSampleType.name}`} />
            <div className="mt-4 mb-4 flex items-center justify-between gap-2 px-4">
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-2xl font-bold">Edit Lab Sample Type</h1>
                </div>
            </div>
