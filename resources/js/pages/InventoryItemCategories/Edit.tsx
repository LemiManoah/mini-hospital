...
export default function InventoryItemCategoryEdit({ inventoryItemCategory }: { inventoryItemCategory: InventoryItemCategory }) {
    const { data, setData, put, processing, errors } = useForm({
        name: inventoryItemCategory.name || '',
        is_active: Boolean(inventoryItemCategory.is_active),
    });

    const handleSubmit = (e: FormEvent) => {
        e.preventDefault();
        put(`/inventory-item-categories/${inventoryItemCategory.id}`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Edit Item Category" />
            <div className="mt-4 mb-4 flex items-center justify-between gap-2 px-4">
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-2xl font-bold">Edit Item Category</h1>
                </div>
            </div>
